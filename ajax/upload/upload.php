...
$nameFile =  transliterateString($_FILES['file']['name']);
$nameFile = preg_replace('/[^a-zA-Z0-9_.]/', '', $nameFile);

/* SI ON VEUX FAIRE UN DOSSIER PARS AGENCE AVEC LEURS IMAGE */
//$urlFile = '/uploads/' .$user.'/'. $nameFile;

// UPLOAD FILE IN FOLDER UPLOAD
$urlFile = '/uploads/'. $nameFile;
$fileSize = formatSizeUnits($_FILES['file']['size']);
$typeFile = $_FILES['file']['type'];
$mysqli = new mysqli("localhost:3306", "root", "", "trashdata");

$sql = "INSERT INTO file_upload (name_file, url_file, size_file, type_file, user_name) VALUES ('$nameFile', '$urlFile', '$fileSize','$typeFile', '$user')";

//S3 VAR SENDED
$urlLocals = $_FILES['file']['tmp_name'];
...
if(!file_exists('../../uploads')){
    ...
    if ($mysqli->query($sql) === TRUE) {
        $urlLocals = $_FILES['file']['tmp_name'];
        $tmpfile = tmpfile();
        $new_temp_location = basename($urlLocals);
        $image = new ImageResize($urlLocals);
        $image->resizeToWidth(1400);
        $image->save($urlLocals);

        copy($urlLocals, $new_temp_location);
        $s3->uploadFile($nameFile, $new_temp_location, $typeFile);
        unlink($new_temp_location);
    } else {
    ...
    if ($mysqli->query($sql) === TRUE) {
        ...
        $image->save($urlLocals);
        copy($urlLocals, $new_temp_location);
        fwrite($tmpfile, file_get_contents($new_temp_location));

        var_dump($new_temp_location);
        var_dump(file_get_contents($new_temp_location));
